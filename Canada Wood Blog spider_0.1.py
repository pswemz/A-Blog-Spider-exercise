...
def wood_blog_spider(verified_URL, blog_tag):

    global file_content, headers, count

    #url = "https://canadawood.org/blog/%s/"  % blog_tag

    source_code = requests.get(verified_URL, headers=random.choice(headers))
...
def page_existence(blog_tag, page_counter):
    global file_content, headers

    url = "https://canadawood.org/blog/%s/page/%d/"  % (blog_tag, page_counter)
    print("Verifying page # %d of tag %s\n" % (page_counter, blog_tag))
    #response = requests.get(url)              #This website has infinity pages without content, so cannot judge through response
    #if response.status_code == requests.codes.not_found:            ##################################
    source_code = requests.get(url, headers=random.choice(headers))
    plain_text = source_code.text
    soup = BeautifulSoup(plain_text)
    if isinstance(soup.find('article'), bs4.element.Tag):
    #if soup.find('article'):
      print("Page # %d of tag %s is valid\n" % (page_counter, blog_tag))
      return url
    else:
        print("No more page of tag %s, the last valid page is # %d\n" % (blog_tag, page_counter-1))
        return None
...
def do_spider(blog_lists):
    ...
    for blog_tag in blog_lists:
        count = 0
        page_counter = 1
        verified_URL = page_existence(blog_tag, page_counter)
        while verified_URL!=None:
          wood_blog_spider(verified_URL, blog_tag)
          page_counter += 1
          verified_URL = page_existence(blog_tag, page_counter)
        #continue
